...
def done_period(row):
    val = row['repeat']
    #logging.info(' __Business__  done_period __> val:'+str(val))
    rp_n = val.split('-')
    repeat = rp_n[0]
    n = 0
    if len(rp_n) >1:
    ...
    #logging.info(' __Business__  done_period __> result:'+str(result))
    print(result)
    return result
...
def get_tasks_list(owner_id,category='not_done & current & start_end & short'):
    logging.info(' __Business__  get_tasks_list __> owner_id:'+str(owner_id)+'| category:'+str(category))
    ...
    if 'not_done' in category and len(tasks_df)>0:
        ...
        print('xxxxxxxx',tasks_df_with_done)
    ...
    logging.info(' __Business__  get_tasks_list __> result:'+str(result))
    return result
...
#     logging.info(' __Business__  reading_busy_time __> result:'+str(df))
#     return df
...
def user_id_list():
    ...
    logging.info(' __Business__  user_id_list __> result:'+str(result))
    return result
...
def change_status(val,text,owner_id):
    ...
    logging.info(' __Business__  change_status __> result:'+str(result))
    return result
    
def adding_task(text,group_id,owner_id):
    logging.info(' __Business__  adding_task __> text:'+str(text)+'| group_id:'+str(group_id)+'| owner_id:'+str(owner_id))
    ...
    logging.info(' __Business__  adding_task __> result:'+str(new_id))
    return new_id


def editing_task(task_id,owner_id):
    logging.info(' __Business__  editing_task __> task_id:'+str(task_id)+'| owner_id:'+str(owner_id))
    df = dl.reading_file('tasks.csv')
    df = df[(df.status=='active') | (df.owner_id!= int(owner_id))]
    ...
    dl.writing_file(df,'tasks.csv')
    logging.info(' __Business__  editing_task __> result: this row has been added:'+str(row))

def updating_setting(update_dict,owner_id):        
    logging.info(' __Business__  updating_setting __> update_dict:'+str(update_dict)+'| owner_id:'+str(owner_id))
    df = dl.reading_file('users.csv')
    ...
    logging.info(' __Business__  updating_setting __> result:'+str(df.loc[df['id'] == int(owner_id)]))



def updating_inactive_task(update_dict,owner_id):        
    logging.info(' __Business__  updating_inactive_task __> update_dict:'+str(update_dict)+'| owner_id:'+str(owner_id))
    df = dl.reading_file('tasks.csv')
    #id	name	time_cost	time	repeat	start	weekend	Why	Periority
    ...
    dl.writing_file(df,'tasks.csv')
    logging.info(' __Business__  updating_inactive_task __> result:'+str(new_task.id))
    return new_task.id
    

def all_task():
    result = 'not available now'
    logging.info(' __Business__  all_task __> result:'+str(result))
    return result


def get_task_info(task_id):
    logging.info(' __Business__  get_task_info __> task_id:'+str(task_id))
    df = dl.reading_file('tasks.csv')
    ...
    result = df.iloc[0]
    logging.info(' __Business__  get_task_info __> result:'+str(result))
    return result


def get_groups(user_id):
    logging.info(' __Business__  get_groups __> user_id:'+str(user_id))
    df = dl.reading_file('user_group.csv')
    df = df[df.user_id == int(user_id)]
    
    result = df[['group_id','branch_name']]
    logging.info(' __Business__  get_groups __> result:'+str(result))
    return result

def get_durations(user_id):
    logging.info(' __Business__  get_durations __> user_id:'+str(user_id))
    df = dl.reading_file('times.csv')
    df = df[df.owner_id == int(user_id)]
    result = df
    logging.info(' __Business__  get_durations __> result:'+str(result))
    return result
...
def add_user_if_not_exist(owner_id,name):
    ...
    if int(owner_id) not in df_user.id.to_list():
        ...
        logging.info(' __Business__  add_user_if_not_exist __> result: this rows has been added:'+str(new_row)+'|\n and this row: '+str(new_user_row))
        
    logging.info(' __Business__  add_user_if_not_exist __> result: user was not new')
    
    
def get_settings_dict(user_id):
    logging.info(' __Business__  add_user_if_not_exist __> owner_id:'+str(user_id))
    df_user = dl.reading_file('users.csv')
    setting_dict = df_user.loc[df_user.id == int(user_id)].drop(['id', 'name'], axis=1).iloc[0]
    logging.info(' __Business__  add_user_if_not_exist __> result: '+str(setting_dict))
    return setting_dict
